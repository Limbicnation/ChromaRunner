...
#if !UE_BUILD_SHIPPING
void USideRunnerGameInstance::DebugTriggerGameOver()
{
...
void USideRunnerGameInstance::DebugSetScore(int32 NewScore)
{
    if (NewScore < 0)
    {
    ...
    UE_LOG(LogTemp, Warning, TEXT("DEBUG: Score set to %d"), CurrentScore);

    if (GEngine)
    {
        GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Yellow,
            FString::Printf(TEXT("DEBUG: Score set to %d"), CurrentScore));
    }
}

void USideRunnerGameInstance::DebugAddLives(int32 LivestoAdd)
{
    if (LivestoAdd <= 0)
    {
    ...
    CurrentLives = FMath::Min(CurrentLives + LivestoAdd, 99); // Cap at 99 lives
    OnLivesUpdated.Broadcast(CurrentLives, MaxLives);

    UE_LOG(LogTemp, Warning, TEXT("DEBUG: Added %d lives - Total: %d/%d"), LivestoAdd, CurrentLives, MaxLives);

    if (GEngine)
    {
        GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Green,
            FString::Printf(TEXT("DEBUG: Lives: %d/%d"), CurrentLives, MaxLives));
    }
}
#endif
